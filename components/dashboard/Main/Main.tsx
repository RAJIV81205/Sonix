...
const Main = () => {
  ...
  const [showPlaylistDropdown, setShowPlaylistDropdown] = useState<
    string | null
  >(null);
  const [recentSongForPlaylist, setRecentSongForPlaylist] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        recentPlaylistDropdownRef.current &&
        !recentPlaylistDropdownRef.current.contains(event.target as Node)
      ) {
                  ...
                  <div className="relative">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowMenuOptions(
                          showMenuOptions === song.id ? null : song.id
                        );
                      }}
        ...
        {playlists.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {playlists.map((playlist, index) => (
              ...
              >
                <div
                  className={`w-full aspect-square mb-3 bg-gradient-to-br ${getPlaylistColor(
                    index
                  )} rounded-lg flex items-center justify-center shadow-lg overflow-hidden`}
                >
                  {playlist.cover ? (
                    <img
                      src={playlist.cover
        ...
        <div className="absolute w-64 z-50 flex flex-col bg-zinc-900 top-16 right-6 rounded-lg border border-zinc-800 shadow-lg overflow-hidden">
          ...
          <div className="p-4 bg-zinc-800 mt-2">
            ...
            <p className="text-xs text-zinc-400 mb-3">
              Unlock all features and enjoy ad-free music!
            </p>
            <button
